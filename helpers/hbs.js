...
module.exports ={
   ...
   let i = (options.hash.current > 5 ? Number(options.hash.current) - 3 : 1) 
...
for (; i <= (Number(options.hash.current) + 4) && i <= options.hash.pages; i++)
{
    ...
    if (i == Number(options.hash.current) + 3 && i < options.hash.pages)
    {
       ...
       {
        outputHTML += `<li class="page-item  ><a class="page-link">Last</a></li>`   
       }

       else{
        outputHTML += `<li class="page-item  disabled><a class="page-link " href="?page=${options.hash.current}">...</a></li>`
       }
